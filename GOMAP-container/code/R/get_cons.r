get_consensus = function(comp_data,obo){
    ...
    comp_dataset = gaf_check_simple(go_obo,comp_dataset)
    ...
    outfile = "comp_data/maize_v3.simp_maj.cons.gaf"
    
    #generate out_gaf with consensus data
    out_gaf = comp_dataset[,get_simple_majority(.SD,aspect,db_object_id,.I,unit_size,go_obo,obs_and_alt),by=list(db_object_id,aspect),]
    ...
    maj_th = 2
    outfile = "comp_data/maize_v3.two_tool.cons.gaf"
    out_gaf = comp_dataset[,get_simple_majority(.SD,aspect,db_object_id,.I,unit_size,go_obo,obs_and_alt),by=list(db_object_id,aspect),]
}
